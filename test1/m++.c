...
int main (int argv, char **argc) {
	...
	int N = 1000;		     	    //number of stars, Mcl will be set to 0 if specified!
	double Mcl = 00.0;               //total mass of the cluster, only used when N is set to 0, necessary for usage of maximum stellar mass relation of Weidner & Kroupa 2007
	int profile = 0;				//density profile; =0 Plummer sphere, =1 King profile
	double W0 = 9.0;				//King's W0 paramter [0.3-12.0]
        double Rh = 0.7;			//cluster half mass radius(pc)
	double tcrit = 1000;			//Simulation time [N-body units (Myr in Nbody6 custom)]
	int tf = 2;						//tidal field: =1 Near-field approximation, =2 point-mass galaxy, =3 Allen & Santillan (1991) MW potential (or Sverre's version of it)
	double RG[3] = {10.0,0.0,0.0}; //Initial Galactic coordinates of the cluster [pc]
	double VG[3] = {0.0,6.55792178,0.0};  //Initial velocity of the cluster [km/s]
	double Q = 0.5;					//Initial virial ratio; =0.5 virial equilibrium, <0.5 collapsing, >0.5 expanding
	
	//Mass function parameters
	int mfunc = 0;					//0 = single mass stars; 1 = use Kroupa (2001) mass function; 2 = use multi power law; 3 = three mass component
	double single_mass = 10.0;		//stellar mass in case of single-mass cluster 
//==========================================================
	int N_o = 1000;	// # of ordinary star; for mfunc=3
...
//============================================================
	...
	double mup = 40.0;				//upper mass limit for mfunc = 1
	...
	double fbin = 0.3;				//Primordial binary fraction number of binary systems = 0.5*N*fbin, only used when nbin is set to 0 
	...
	char *output = "1e4Mcl_fbin0.01";			//name of output files
	...
	double upper_IMF_limit = 40.0; //Maximum stellar mass allowed in mcluster [Msun]
