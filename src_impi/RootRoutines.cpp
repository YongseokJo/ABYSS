    ...
    int pid[NumberOfWorker];  
		...
		while (completed_tasks < total_tasks) {
			...
			if (remaining_tasks > 0) {
				...
				MPI_Ssend(&task,      1, MPI_INT, completed_rank, TASK_TAG, MPI_COMM_WORLD);
				MPI_Ssend(&ptcl_id,   1, MPI_INT, completed_rank, PTCL_TAG, MPI_COMM_WORLD);
				...
				while (completed_tasks < total_tasks) {
					...
					*/
					...
					if (remaining_tasks > 0) {
						...
						MPI_Ssend(&task,      1, MPI_INT, completed_rank, TASK_TAG, MPI_COMM_WORLD);
						MPI_Ssend(&ptcl_id,   1, MPI_INT, completed_rank, PTCL_TAG, MPI_COMM_WORLD);
						MPI_Ssend(&next_time, 1, MPI_DOUBLE, completed_rank, TIME_TAG, MPI_COMM_WORLD);
						remaining_tasks--;
					} else {
						//printf("Rank %d: No more tasks to assign\n", completed_rank);
					}
