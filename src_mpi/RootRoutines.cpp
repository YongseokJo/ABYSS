...
void RootRoutines(int num_workers, std::vector<int> tasks) {
	...
	int task = 0, task_recv, num_tasks;
	...
	{
		task = 9;
		num_tasks = NumberOfParticle;

		// Initial assignments
		worker_rank = 1;
		for (ptcl_id=0; ptcl_id<num_workers; ptcl_id++) {
			if (num_tasks < ptcl_id)
				break;
		...
		while (completed_tasks < num_tasks) {
			...
			if (remaining_tasks > 0) {
				ptcl_id = num_workers + completed_tasks;
	...
	/* Receive the restuls */
	/*
	while (next_task < num_tasks || active_workers > 0) {
		int worker_rank;
		MPI_Status status;
