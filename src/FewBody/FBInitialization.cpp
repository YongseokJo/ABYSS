...
void Group::initialManager() {
	...
	manager.time_step_min = 1e-14; // minimum physical time step // 1e-13 in ar.cxx
...
void Group::initialIntegrator() {
	...
	sym_int.info.r_break_crit = 1e-4/position_unit; // distance criterion for checking stability
...
// New test starts
	...
	for (Particle* member : ptclGroup->Members) {
		for (Particle* neighbor : member->ACList) {
			...
			if (std::find(ptclCM->ACList.begin(), ptclCM->ACList.end(), neighbor) == ptclCM->ACList.end() &&
				std::find(ptclGroup->Members.begin(), ptclGroup->Members.end(), neighbor) == ptclGroup->Members.end()) {
...
// Test starts
	...
	// // /* // Eunwoo added
	// while (ptclCM->TimeStepReg*EnzoTimeStep*std::sqrt(ptclCM->Velocity[0]*ptclCM->Velocity[0]+ptclCM->Velocity[1]*ptclCM->Velocity[1]+ptclCM->Velocity[2]*ptclCM->Velocity[2]) > ptclCM->RadiusOfAC) {
	// 	ptclCM->TimeLevelReg--;
	// 	ptclCM->TimeStepReg = static_cast<REAL>(pow(2, ptclCM->TimeLevelReg));
	// 	ptclCM->TimeBlockReg = static_cast<ULL>(pow(2, ptclCM->TimeLevelReg-time_block));
	// }
