...
class TidalTensor{
private:
    REAL T1[3];     // 0 constant 
    REAL T2[9];  // 1st (9)    general tensor
    //PS::F64 T2[6];  // 1st (6)  symmetry tensor
#ifdef TIDAL_TENSOR_3RD
    REAL T3[10]; // 2nd Tensor (10)
#endif
public:
    REAL Position[Dim];  // position of c.m.
    ...
    static void createTidalTensorMeasureParticles(Particle* _ptcl_tt, const Particle& _ptcl_cm, const REAL _size) {
#ifdef TIDAL_TENSOR_3RD
        ///* Assume _size is the maximum length inside box
        //   Then the edge length=_size/(2*sqrt(2))
        // */

        REAL lscale = 0.16*_size; // half edge size
        ...
        REAL lscale = 0.5*_size; // half edge size
    ...
    */
    // template<class Tptcl>
    void fit(Particle* _ptcl_tt, Particle& _ptcl_cm,  const REAL _size) {
        // get c.m. position
     ...
     */
    void shiftCM(const REAL (&_pos)[3]) {
#ifdef TIDAL_TENSOR_3RD
        REAL dr[0] = _pos[0]-Position[0];
        REAL dr[1] = _pos[1]-Position[1];
        REAL dr[2] = _pos[2]-Position[2];
    ...
    void eval(REAL (&acc)[3], const REAL (&pos)[3]) const {
        REAL acc0=acc[0];
        REAL acc1=acc[1];
        REAL acc2=acc[2];
    ...
    REAL evalPot(const REAL (&pos)[3]) const {
        REAL x = pos[0];
        REAL y = pos[1];
        REAL z = pos[2];
#ifdef TIDAL_TENSOR_3RD
        ...
        REAL acc0 =  T1[0] + 0.5*(T2[0]*x + T2[1]*y + T2[2]*z) 
            +      (T3[0]*x2 + 2*T3[1]*xy + 2*T3[2]*xz + T3[3]*y2 + 2*T3[4]*yz + T3[5]*z2)/3.0;
        REAL acc1 =  T1[1] + 0.5*(T2[3]*x + T2[4]*y + T2[5]*z)
            +      (T3[1]*x2 + 2*T3[3]*xy + 2*T3[4]*xz + T3[6]*y2 + 2*T3[7]*yz + T3[8]*z2)/3.0;
        REAL acc2 =  T1[2] + 0.5*(T2[6]*x + T2[7]*y + T2[8]*z)
