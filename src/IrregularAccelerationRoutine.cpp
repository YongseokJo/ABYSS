...
#define binary
	...
	_time.irr_bin.markStart();
		...
		if ((GroupList.size()>0)) { //&(binary_time_prev != binary_time)) {
			//fprintf(binout, "-------------------------------------\n");
			...
			//fflush(stderr);
			//fflush(binout);
			GroupAccelerationRoutine(ComputationList[0]->CurrentTimeIrr+ComputationList[0]->TimeStepIrr, particle); // Eunwoo: is this right?
